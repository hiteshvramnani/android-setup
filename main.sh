...
function burpcer(){
    ...
    if [[ "$cert_check" == "9a5ba575.0" ]]; then
        echo -e "\033[1;91m"
        echo -e "Already Burpsuite Certificate found, this will replace the existing one\n"
    ...
    if [[ $res == 'N' || $res == 'n' ]]; then
        ...
        name=$(openssl x509 -inform PEM -subject_hash_old -in cacert.pem | head -1).0
        mv cacert.pem $name
...
################ All necessary functions defined above ###################

function banner(){
    #Color change  every time
